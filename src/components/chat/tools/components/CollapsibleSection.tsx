  ...
  return (
    <details className={`group/details relative ${className}`} open={open}>
      <summary className="flex cursor-pointer select-none items-center gap-1.5 py-0.5 text-xs group-open/details:sticky group-open/details:top-0 group-open/details:z-10 group-open/details:-mx-1 group-open/details:bg-background group-open/details:px-1">
        ...
        <svg
          className="h-3 w-3 flex-shrink-0 text-gray-500 transition-transform duration-150 group-open/details:rotate-90 dark:text-gray-500"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
        ...
        {toolName && (
          <span className={`flex-shrink-0 font-medium ${isExecuting ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-500'}`}>{toolName}</span>
        )}
        {toolName && (
        ...
        {onTitleClick ? (
          ...
          <span className="flex-1 truncate text-gray-600 dark:text-gray-500">
            {title}
          </span>
        )}
